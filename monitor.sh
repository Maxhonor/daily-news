...
def negative_screen_send(markdown, task_name):
    ...
    try:
        trace_id = f"sszs-{task_name.replace(chr(32),chr(45))}-{now_ts}"
        wrapped = {"data": task_data}
        req = urllib.request.Request(NEGATIVE_SCREEN_URL,
            data=json.dumps(wrapped, ensure_ascii=False).encode(),
            headers={"Content-Type": "application/json", "x-trace-id": trace_id})
        resp = json.loads(urllib.request.urlopen(req, timeout=10).read())
        rsp_code = resp.get('code', '?')
        rsp_desc = resp.get('desc', resp.get('message', 'OK'))
        print(f"负一屏推送: {rsp_code} {rsp_desc}")
        if rsp_code != '0000000000':
            print(f"  详情: {json.dumps(resp, ensure_ascii=False)}")
