...
- 直接输出翻译结果，不要编号，不要多余说明"""
    ...
    translated = resp.get("output", {}).get("text", "").strip()
    if not translated and "choices" in resp.get("output", {}):
...
trace_id = f"morning-news-{now_ts}"
wrapped = {"data": task_data}
req1 = urllib.request.Request(push_url, data=json.dumps(wrapped, ensure_ascii=False).encode(),
    headers={"Content-Type": "application/json", "x-trace-id": trace_id})
try:
    resp1 = json.loads(urllib.request.urlopen(req1, timeout=10).read())
    rsp_code = resp1.get('code', '?')
    rsp_desc = resp1.get('desc', resp1.get('message', 'OK'))
    print(f"负一屏推送: {rsp_code} {rsp_desc}")
    if rsp_code != '0000000000':
        print(f"  可能需要检查: {json.dumps(resp1, ensure_ascii=False)}")
